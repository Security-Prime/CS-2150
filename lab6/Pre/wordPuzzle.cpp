// fr2md 	Farid Rajabi Nia	wordPuzzle.cpp
...
void printT(int row, int col , int dir);
...
int main (int argc, char **argv) {
	...
	if (argc!=3){
		cout<<"<dictionary_file> <grid_file>";
		return 1;
	}
	...
	string testline;
	ifstream Test ( string(argv[1]).c_str() );
	...
	for (int j=0; j<=rows; j++){
		for (int k=0; k<=cols; k++){
			for (int d=0; d<8; d++){
				for (int l=3; l<10; l++){
					string s = getWordInGrid(j,k,d,l,rows,cols);
					if (s.length() == l){
						if (hashT.findWord(s)){
							printT(j,k,d);
							cout<<s<<endl;
	...
	cout<<count<<" words were found"<<endl;
	t.stop();
